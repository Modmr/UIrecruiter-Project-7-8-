...
<section id ="app">
    ...
    <!-- The input -->
    <div class="query">
        <span class="app-title">Recruiter Diana</span>
    ...
    <main class="wrapper ai-window">
        ...
        <table v-for="a in answers" class="chat-window">
            ...
            <tr>
                ...
                <span v-if="(a.result.metadata.intentName == 'Step6 - Get Availability')">{{outdata()}}</span>
                <span v-if="(a.result.metadata.endConversation == true)">{{callDecision()}}</span>
            </tr>
        ...
        <br>
        <p class="copyright" v-if="answers.length > 0 && config.app.watermark">00Proudly powered by <a href="https://mish.io">Ushakov</a> & <a href="https://dialogflow.com">Dialogflow</a></p>
        <a id="bottom"></a>
    </main>
</section>
</template>
...
export default {
    ...
    methods: {
        ...
        handle(response){
            this.savedata(response.result.metadata.intentName, response.result)
            ...
            if(response.result.fulfillment.speech || response.result.fulfillment.messages[0].type == 'simple_response'){
                ...
                speech.voice = voices.filter(function(voice) { return voice.name == 'Tessa'; })[0];
                
                speech.voiceURI = 'Tessa'
        ...
        savedata(intentName, sdata){
            
            if(intentName == 'Step6 - Get Availability') {
                this.userdata.name = sdata.contexts[0].parameters['given-name']
                this.userdata.age = sdata.contexts[0].parameters['age.original']
                this.userdata.degree = sdata.contexts[0].parameters['college-degree']
                this.userdata.traits_positive = sdata.contexts[0].parameters['traits-positive']
                this.userdata.traits_negative = sdata.contexts[0].parameters['traits-negative']
                this.userdata.traits_neutral = sdata.contexts[0].parameters['traits-neutral']
                this.userdata.skills = sdata.contexts[0].parameters['skills']
                this.userdata.position = sdata.contexts[0].parameters['position.original']
                this.userdata.exptime = sdata.contexts[0].parameters['experience.original']
                this.userdata.hours = sdata.contexts[0].parameters['hours']                
